...
| 26 | 2026-09-02 | Bounded-concurrent evidence collection (ThreadPoolExecutor, one flat pool per collection run) instead of sequential | `/api/operations/brief?refresh=true` sequentially collecting 22 Azure sources exceeded a 300s Gunicorn worker timeout (502 gateway error). Concurrency bounded by `OPERATIONS_COLLECTION_MAX_WORKERS` (default 6, hard cap 12) preserves deterministic envelope order and per-source error isolation while cutting wall-clock time roughly proportionally to the worker count. |
