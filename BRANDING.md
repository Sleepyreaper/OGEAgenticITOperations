# Branding Guide — DTE Cloud Weather Ops

This repo is a **white-label** multi-agent AI operations platform, pre-configured with DTE Energy branding. Clone it and customize the values below to rebrand it for your customer.
...
- `- logo text` — the nav logo text (line ~59)
- `Microsoft · DTE Energy` — the subtitle (line ~62)
- `DTE Cloud Weather Ops` — the app name (appears in `<title>` and headers)
...
|------------|-------------|-------------|
| `orchestrator` | Grid Dispatch | Routes requests, synthesizes answers |
| `cost_sentinel` | Meter Reader | Cost optimization |
| `standards_architect` | The Lineman | Infrastructure standards |
| `diagnostics_sre` | Blackout | Diagnostics & root cause |
| `scout` | Arc Flash | Proactive monitoring |
| `compliance_inspector` | The Regulator | Compliance checking |
...
Edit `package.json`:
```json
"name": "dte-weather-ops"  // ← Change to customer project name
```
